...
| - | 单元测试 | 1 天 | P0 |
| **阶段 3** | D3D11Renderer 实现 | 2-3 天 | P0 | ✅ 完成 |
| - | D3D11Renderer 主类 | 1 天 | P0 | ✅ 完成 |
| - | 零拷贝流水线 | 1 天 | P0 | ✅ 完成 |
| - | 单元测试 | 1 天 | P0 | ⬜ 待完成 |
| **阶段 4** | ZenPlayer 集成 | 2-3 天 | P0 | ✅ 完成 |
| - | 渲染路径选择逻辑 | 1 天 | P0 | ✅ 完成 |
| - | 配置系统集成 | 1 天 | P0 | ✅ 完成 |
| - | 降级和错误处理 | 1 天 | P0 | ✅ 完成 |
| **阶段 5** | 测试与优化 | 3-5 天 | P0 |
...
1. **D3D11Renderer 主类实现** ✅
   - 文件：`src/player/video/render/impl/d3d11_renderer.h`
   - 文件：`src/player/video/render/impl/d3d11_renderer.cpp`
   - 功能：
     - 初始化 D3D11 设备和上下文
     - 共享解码器设备支持
     - 窗口大小调整支持
...
2. **零拷贝渲染流水线** ✅
   - 从 AVFrame 直接提取 D3D11 纹理（零拷贝）
   - 创建 NV12 格式的着色器资源视图（Y 平面 + UV 平面）
   - GPU 端 YUV→RGB 颜色空间转换
   - 全屏四边形渲染（无顶点缓冲，使用 SV_VertexID）

3. **核心特性** ✅
   - 支持硬件解码输出格式（AV_PIX_FMT_D3D11）
   - 与解码器共享 D3D11 设备，避免跨设备传输
   - 使用 D3D11Context、D3D11Shader、D3D11SwapChain 组件
   - 完整的错误处理和日志记录
...
// 零拷贝核心代码示例
Result<void> D3D11Renderer::RenderFrame(AVFrame* frame) {
  // 1. 直接从 AVFrame 提取 GPU 纹理（零拷贝）
  ID3D11Texture2D* decoded_texture = 
      reinterpret_cast<ID3D11Texture2D*>(frame->data[0]);
  
  // 2. 创建着色器资源视图（零拷贝，只是创建"视图"）
  CreateShaderResourceViews(frame);
  
  // 3. GPU 端渲染（数据一直在 GPU 内存）
  RenderQuad();
...
D3D11Renderer 依赖以下组件（阶段1已完成）：
- ✅ `D3D11Context` - 设备和上下文管理
- ✅ `D3D11Shader` - YUV→RGB 着色器
- ✅ `D3D11SwapChain` - 交换链管理

#### 性能预期

| 指标 | 软件路径 (SDL) | 硬件路径 (D3D11Renderer) | 预期提升 |
|------|---------------|------------------------|---------|
| CPU 使用率 | 30-60% | 5-10% | 🚀 6倍 |
| 内存拷贝 | 2-3次/帧 | 0次/帧 | ✅ 零拷贝 |
| 帧延迟 | 16-33ms | 8-16ms | ⚡ 减半 |
| 4K 支持 | 卡顿 | 流畅 | 🎬 可播放 |
...
**最后更新**: 2025-10-28  
**设计状态**: ✅ 完成
**实施状态**: 🚧 进行中（阶段3已完成 D3D11Renderer 实现）

**阶段3完成内容**：
1. ✅ D3D11Renderer 主类实现（零拷贝渲染）
2. ✅ 零拷贝流水线（GPU纹理直接绑定）
3. ⬜ 单元测试（待完成）

**已实现文件**：
- ✅ `src/player/video/render/impl/d3d11_renderer.h` - D3D11渲染器接口
- ✅ `src/player/video/render/impl/d3d11_renderer.cpp` - D3D11渲染器实现
