...
```bash
# 显示每一帧的 PTS 和持续时间
ffprobe -show_frames -show_entries frame=pkt_pts,pkt_duration,pict_type \
        -select_streams v:0 input.mp4 | grep -E "pkt_pts|pkt_duration|pict_type"
```

**输出示例**（30 fps 视频，时间基 1/90000）：
```
pict_type=I
pkt_pts=0
pkt_duration=3003
...
```bash
# 提取视频和音频的时间戳信息
ffprobe -show_frames -select_streams v:0 -show_entries frame=pkt_pts_time \
        input.mp4 > video_pts.txt

ffprobe -show_packets -select_streams a:0 -show_entries packet=pts_time \
        input.mp4 > audio_pts.txt
```

**分析**：
```bash
# 查看前 10 个视频帧的 PTS（秒）
head -20 video_pts.txt | grep pts_time
```
...
```bash
# 查看前 10 个音频包的 PTS（秒）
head -20 audio_pts.txt | grep pts_time
```
...
**同步检查**：
```
视频 PTS: 0.000, 0.033, 0.067, 0.100 ...
音频 PTS: 0.000, 0.023, 0.046, 0.069, 0.093 ...

同步点:
  t=0 ms:   视频帧 0 和音频包 0 对齐 ✅
  t=100 ms: 视频帧 3 (PTS=0.100) 和音频包 4 (PTS=0.093) 接近 ✅
```
...
```bash
# 音频延迟 500 ms（音频比视频慢）
ffmpeg -i input.mp4 -itsoffset 0.5 -i input.mp4 -map 0:v -map 1:a \
       -c copy output_audio_delay.mp4

# 视频延迟 500 ms（视频比音频慢）
ffmpeg -i input.mp4 -itsoffset 0.5 -i input.mp4 -map 1:v -map 0:a \
       -c copy output_video_delay.mp4
```
...
```bash
# 方法 1：直接调整 PTS
ffmpeg -i input.mp4 -itsoffset -0.2 -i input.mp4 -map 0:v -map 1:a \
       -c copy output_fixed.mp4

# 方法 2：音频裁剪（删除前 200 ms）
ffmpeg -i input.mp4 -ss 0.2 -i input.mp4 -map 0:v -map 1:a \
       -c copy output_fixed2.mp4
```

---

### 实验 5：可视化时间戳（Python 脚本）
...
cmd = [
    ...
    '-show_frames', '-select_streams', 'v:0', 'input.mp4'
]
...
pts_list = []
frame_types = []
for frame in data['frames'][:100]:  # 前 100 帧
    pts_list.append(float(frame['pkt_pts_time']))
    frame_types.append(frame['pict_type'])

# 绘图
colors = {'I': 'green', 'P': 'blue', 'B': 'orange'}
for i, (pts, ftype) in enumerate(zip(pts_list, frame_types)):
    plt.scatter(i, pts, color=colors[ftype], label=ftype if i == 0 else "")

plt.xlabel('Frame Index')
plt.ylabel('PTS (seconds)')
plt.title('Video Frame PTS Timeline')
plt.legend(['I Frame', 'P Frame', 'B Frame'])
plt.grid(True, alpha=0.3)
plt.savefig('pts_timeline.png', dpi=150)
print("图表已保存到 pts_timeline.png")
```

📊 **配图位置 5：PTS 时间线可视化示例**
> **可灵 AI 绘图提示词**：
> ```
> 散点图风格，白色背景，16:9横版。横轴标注"Frame Index"刻度0到100，纵轴标注"PTS (seconds)"刻度0到3.5。散点分三种颜色：绿色大圆点标注"I Frame"（每隔30个点出现一次），蓝色中圆点标注"P Frame"（每隔10个点出现一次），橙色小圆点标注"B Frame"（其余位置密集分布）。散点形成一条上升的直线趋势，体现时间线性增长。右上角图例显示三种帧类型。整体风格：科学绘图风格，浅灰色网格线，Arial字体。
> ```
...
**现象**：
```
packet->pts = AV_NOPTS_VALUE  // -9223372036854775808 (INT64_MIN)
```
...
if (frame->pts == AV_NOPTS_VALUE) {
  if (frame->best_effort_timestamp != AV_NOPTS_VALUE) {
    frame->pts = frame->best_effort_timestamp;  // FFmpeg 的估算值
  } else {
    // 手动计算：当前时间 + 累计时长
    frame->pts = last_pts_ + frame_duration_;
  }
}
last_pts_ = frame->pts;
```
...
**现象**：
```
Frame 10: PTS = 1000 ms
Frame 11: PTS = 950 ms  ← 倒退了！
```
...
```cpp
if (frame->pts < last_pts_) {
  LOG_WARN("PTS going backwards: {} -> {}", last_pts_, frame->pts);
  
  // 策略 1：强制递增
  frame->pts = last_pts_ + frame_duration_;
  
  // 策略 2：丢弃这一帧
  // return Result<AVFrame*>::Err("Invalid PTS");
}
```
...
**现象**：
```
视频流: time_base = 1/90000
音频流: time_base = 1/44100
容器:   time_base = 1/1000
```
...
```cpp
// FFmpeg 的时间基转换 API
int64_t video_pts_ms = av_rescale_q(
  video_packet->pts,           // 原始 PTS
  video_stream->time_base,     // 源时间基 (1/90000)
  {1, 1000}                    // 目标时间基 (1/1000 = 毫秒)
);

int64_t audio_pts_ms = av_rescale_q(
  audio_packet->pts,           // 原始 PTS
  audio_stream->time_base,     // 源时间基 (1/44100)
  {1, 1000}                    // 目标时间基 (1/1000 = 毫秒)
);

// 现在可以直接比较
int64_t diff = video_pts_ms - audio_pts_ms;
```
